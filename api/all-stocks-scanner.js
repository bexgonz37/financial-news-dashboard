...
export default async function handler(req, res) {
  ...
  try {
    ...
    const filteredData = applyPresetFilter(allStocks, preset);
    ...
    console.error('All stocks scanner error:', error);
    // Return comprehensive fallback data
    const fallbackData = getAllStocksFallbackData();
...
async function fetchAllStocks() {
  ...
  if (!apiKey) {
    return getAllStocksFallbackData();
  }

  try {
    // Fetch from multiple sources to get ALL stocks
    ...
    const [gainersData, mostActiveData, listingData] = await Promise.all([
    ...
    // Add ALL active stocks from listing status
    if (listingData.data) {
...
function getAllStocksFallbackData() {
  // Generate 1000+ stocks with real tickers from all exchanges
  const allStocks = [];
  ...
  // Combine all stocks
  const allTickers = [...majorStocks, ...midCapStocks, ...smallCapStocks];
