...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get LIVE stocks from multiple sources including new listings
    const liveStocks = await fetchLiveStocks();
    
    // Apply preset filters
    const filteredData = applyPresetFilter(liveStocks, preset);
    ...
    console.error('Live stocks scanner error:', error);
    // Return comprehensive fallback data
    const fallbackData = getLiveStocksFallbackData();
...
async function fetchLiveStocks() {
  ...
  if (!apiKey) {
    return getLiveStocksFallbackData();
  }

  try {
    // Fetch from multiple sources to get ALL live stocks including new listings
    ...
    if (gainersData['Information']) {
      console.log('Alpha Vantage rate limit reached, using live fallback');
      return getLiveStocksFallbackData();
    }
    ...
    // Add ALL active stocks from listing status (this includes new IPOs)
    if (listingData.data) {
...
function getLiveStocksFallbackData() {
  // Generate 2000+ stocks with real tickers from all exchanges
  const allStocks = [];
  
  // Major stocks (S&P 500)
  const majorStocks = [
  ...
  // Combine all stocks
  const allTickers = [...majorStocks, ...midCapStocks, ...smallCapStocks, ...recentIPOs, ...cryptoStocks, ...biotechStocks];
