...
export async function scanStocks(preset = 'momentum', limit = 50, filters = {}) {
  ...
  // Get quotes in batches with progress tracking
  const symbols = filteredSymbols.map(s => s.symbol);
  console.log(`Fetching quotes for ${symbols.length} symbols in batches of 50...`);
  
  const quotes = await getQuotesBatched(symbols, 50);
  console.log(`Retrieved ${quotes.length} quotes from ${symbols.length} requested symbols`);
