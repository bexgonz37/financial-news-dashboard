...
class ProviderManager {
  constructor() {
    ...
    this.rateLimiters = new Map(); // provider -> { tokens: 0, lastRefill: 0, maxTokens: 60 }
    this.circuitBreakers = new Map(); // provider -> { failures: 0, lastFailure: 0, state: 'closed' }
    ...
    // AlphaVantage removed from intraday usage - only for daily bars as fallback
    ...
    for (const [name, provider] of this.providers) {
      this.rateLimiters.set(name, { tokens: 60, lastRefill: Date.now(), maxTokens: 60 });
  ...
  isCircuitOpen(providerName) {
    ...
    if (breaker.state === 'open' && timeSinceLastFailure < 60000) { // 60s cooldown
      return true;
    }
  ...
  // Get active providers (not in backoff, circuit open, or inactive)
  getActiveProviders() {
    const now = Date.now();
    const active = [];
    ...
    for (const [name, provider] of this.providers) {
      ...
      if (this.isCircuitOpen(name)) {
        console.log(`Provider ${name} circuit breaker is open`);
        continue;
      }
  ...
  // Handle provider response with circuit breaker logic
  handleProviderResponse(providerName, success, statusCode = 0, error = null) {
    ...
    if (success) {
      ...
      if (breaker) {
        ...
        // Open circuit after 5 consecutive failures or 429/5xx errors
        if (breaker.failures >= 5 || statusCode >= 429) {
          breaker.state = 'open';
  ...
  // Get detailed provider health for UI
  getProviderHealth() {
    const health = {};
    ...
    for (const [name, provider] of this.providers) {
      ...
      if (breaker && breaker.state === 'open') {
        status = 'offline';
        nextRetry = new Date(breaker.lastFailure + 60000).toISOString();
      } else if (this.inactiveProviders.has(name)) {
        status = 'degraded';
        nextRetry = new Date(this.inactiveProviders.get(name)).toISOString();
      } else if (limiter && limiter.tokens <= 5) {
        status = 'degraded';
      }
      ...
      health[name] = {
        ...
        tokensRemaining: limiter?.tokens || 0
      };
    }
