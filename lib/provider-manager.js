...
import { FallbackNewsProvider } from './providers/fallback-news.js';
import { FallbackQuotesProvider } from './providers/fallback-quotes.js';
...
class ProviderManager {
  ...
  initializeProviders() {
    ...
    this.providers.set('fallback-news', new FallbackNewsProvider());
    this.providers.set('fallback-quotes', new FallbackQuotesProvider());
  ...
  getActiveProviders() {
    ...
    for (const [name, provider] of this.providers) {
      // Fallback providers are always active
  ...
  async getQuotes(symbols) {
    ...
    // If no real providers worked, use fallback
    if (quotes.length === 0) {
      try {
        const fallbackProvider = this.providers.get('fallback-quotes');
        const result = await fallbackProvider.getQuotes(symbols);
        if (result && result.length > 0) {
          quotes = result;
          console.log(`Fallback provider returned ${result.length} quotes`);
        }
      } catch (error) {
  ...
  // Get news with fallback (Finnhub + FMP + Fallback)
  async getNews(params) {
    ...
    newsProviders.push({ name: 'fallback-news', provider: this.providers.get('fallback-news') });
