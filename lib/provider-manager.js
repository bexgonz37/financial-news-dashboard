...
class ProviderManager {
  ...
  async getQuotes(symbols) {
    ...
    for (const { name, provider } of activeProviders) {
      if (name.startsWith('fallback-')) continue; // Skip fallback providers
    ...
    if (quotes.length === 0) {
      console.log('No real providers available, returning empty quotes array');
  ...
  // Get news with fallback (Finnhub + FMP only, no fake data)
  async getNews(params) {
    ...
    if (allNews.length === 0) {
      console.log('No real news providers available, returning empty news array');
