...
class SingleTickerResolver {
  ...
  async performResolution(article) {
    ...
    // Stage 0: Provider symbols (highest confidence)
    if (article.symbols && Array.isArray(article.symbols) && article.symbols.length > 0) {
      const symbol = comprehensiveSymbolMaster.getSymbol(article.symbols[0]);
      if (symbol && symbol.isActive) {
        return {
          ticker: article.symbols[0].toUpperCase(),
          confidence: 0.95,
          reason: 'provider',
          isGeneral: false
        };
    ...
    // Stage 1: Source tickers (if present) - high confidence
  ...
  selectPrimaryTicker(candidates) {
    ...
    // Stage 3: FMP search fallback (throttled)
    try {
      const searchResult = await this.searchFMP(combinedText);
  ...
  async searchFMP(query) {
    ...
    try {
      const response = await fetch(`https://financialmodelingprep.com/api/v3/search?query=${encodeURIComponent(query)}&limit=5&apikey=${apiKey}`);
      ...
      const result = data[0];
      const symbol = comprehensiveSymbolMaster.getSymbol(result.symbol);
      if (symbol && symbol.isActive) {
        return {
