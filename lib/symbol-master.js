...
// Supplemental aliases for common mega-caps only (used as fallback, not primary signal)
...
export async function loadSymbolMaster() {
  ...
  try {
    ...
    // If no symbols loaded, use a minimal fallback to prevent complete failure
    if (symbolArray.length === 0) {
      console.warn('No symbols loaded from APIs, using minimal fallback');
      const fallbackSymbols = [
        { symbol: 'AAPL', companyName: 'Apple Inc.', exchange: 'NASDAQ', isActive: true, aliases: ['Apple', 'Apple Inc'], sector: 'Technology', industry: 'Consumer Electronics', type: 'stock', source: 'Fallback' },
        { symbol: 'MSFT', companyName: 'Microsoft Corporation', exchange: 'NASDAQ', isActive: true, aliases: ['Microsoft', 'Microsoft Corp'], sector: 'Technology', industry: 'Software', type: 'stock', source: 'Fallback' },
        { symbol: 'GOOGL', companyName: 'Alphabet Inc.', exchange: 'NASDAQ', isActive: true, aliases: ['Google', 'Alphabet'], sector: 'Communication Services', industry: 'Internet', type: 'stock', source: 'Fallback' },
        { symbol: 'AMZN', companyName: 'Amazon.com Inc.', exchange: 'NASDAQ', isActive: true, aliases: ['Amazon', 'Amazon.com'], sector: 'Consumer Discretionary', industry: 'E-commerce', type: 'stock', source: 'Fallback' },
        { symbol: 'TSLA', companyName: 'Tesla Inc.', exchange: 'NASDAQ', isActive: true, aliases: ['Tesla', 'Tesla Inc'], sector: 'Consumer Discretionary', industry: 'Automotive', type: 'stock', source: 'Fallback' }
      ];
      symbolMaster = fallbackSymbols;
    } else {
...
function generateAliases(companyName, symbol) {
  ...
  // Add supplemental aliases only for mega-caps (as fallback, not primary signal)
  if (SUPPLEMENTAL_ALIASES[symbol]) {
    SUPPLEMENTAL_ALIASES[symbol].forEach(alias => aliases.add(alias));
  }
